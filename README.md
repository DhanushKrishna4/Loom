...
The project is not a git repository yet. To use the workflow:

```bash
git init && git add . && git commit -m "nano-infer"
git remote add origin git@github.com:<you>/<repo>.git
git push -u origin main
```

Then enable Pages with **Source: GitHub Actions** in the repository settings. The
workflow is written but has never run — CI syntax that has not executed is a
guess, however careful, so expect one or two rounds of fixing on the first push.
